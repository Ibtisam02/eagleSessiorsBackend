...
const deleteReviewAdmin = asyncHandler(async (req, res, next) => {
    ...
    if (!review) {
      return next(new ErrorHandler(`No Review Exist for id ${req.params.id}`, 404))
    }
...
const addBannerAdmin = asyncHandler(async (req, res, next) => {
   ...
   await Banner.create({image:image,title:req.body.title,buttonText:req.body.buttonText,description:req.body.description,subTitle:req.body.subTitle,link:req.body.link})
  res.status(200).json({
    success: true,
...
let sendEmailForMsg=asyncHandler(async (req, res, next) => {
  if (!req.body.name || !req.body.email || !req.body.comment) {
    return next(new ErrorHandler(`Cannot send Massege all name, email and comment are required fields`, 401))
  }
  if(!validator.isEmail(req.body.email)){
    return next(new ErrorHandler(`${req.body.email} is not a valid email`, 401))
  }
  await sendEmail({
    email: "ibtisamafzal00@gmail.com",
...
let logo={
  ...
  let logoUpdated=await Logo.findByIdAndUpdate("6783c5a8d85926e871cc3d93",{logo:logo})
...
export { userAddAReview, getAllReviews, getAllReviewsAdmin,deleteReviewAdmin,addTestiMonialsAdmin,getAllTestimonials,deleteTestimonial,addBannerAdmin,deleteBanner,getAllBanners, sendEmailForMsg,changeLogo, getLogo };
